// scripts/rescore-timing-quality.mjs
//
// npm run rescore:timing
//
// 이미 저장된 timing-quality 결과에 수정된 검증기를 다시 적용한다 — API 호출 없음, 비용 0원.
// scripts/rescore-targeted-quality.mjs와 완전히 같은 패턴, TIMING_QUESTIONS만 다르다.
...
const SCOPE_BY_FILE = Object.fromEntries(TIMING_QUESTIONS.map((q) => [q.file, q.scopeCheck]));

const targetDir = process.argv[2] ?? './tests/real-ai/timing-quality';
...
for (const filename of files) {
  ...
  const q = TIMING_QUESTIONS.find((qq) => qq.file === baseName);
  if (!q) {
    console.log(`[${baseName}] timing-questions.mjs에 없는 파일 — 건너뜀`);
    continue;
  }
