...
`ProductsScreen`은 이미 `autoBuyCode`가 있으면 상품 로드 즉시 자동으로 Toss 결제창을
여는 로직을 갖고 있었다(의도한 동작). 다만 **상품 목록이 로드되고 자동결제가 트리거되는
그 짧은 사이에, 전체 상품 리스트(`products?.map`)가 항상 화면에 먼저 그려지고 있어서**,
사용자 입장에서는 "일단 상품안내 화면에 갔다"고 느껴지고 있었다.

### 수정
`autoBuyCode`가 있을 때는 상품 목록을 전혀 렌더링하지 않고, "결제 화면으로 이동하고
있어요..." 전용 로딩 문구만 보여준다. 목록 자체가 화면에 나타나지 않으므로, 사용자가
채팅에서 990원/4,900원 버튼을 누르면 (로그인 경유 여부와 무관하게) 곧바로 결제창으로
이어지는 것처럼 느껴진다. `autoBuyCode`가 없는 일반 진입(마이페이지→상품안내)은 기존과
동일하게 전체 목록을 보여준다(회귀 없음).

### 2. "다른 대화는 여전히 앞뒤가 안 맞는다" — ROOT CAUSE 확정
Render `/health` 응답에서 확인: `"casualModel":"disabled — using casual-response-engine
(rule-based, no API calls)"`. **일상 대화(casual)에 실제 AI가 전혀 연결되어 있지 않고,
미리 정해둔 고정 응답 뱅크만 무작위로 돌려주고 있었다.** 이건 코드 버그가 아니라
`OPENAI_CASUAL_MODEL` 환경변수가 Render production에 설정되어 있지 않은 것이 원인 —
사장님께 Render Environment에 `OPENAI_API_KEY`/`OPENAI_CASUAL_MODEL` 추가를 안내함.

### 신규 검증한 사실 (지난 라운드 이슈 마무리)
"카드가 빈 상자로 보이는" 문제는 실제로는 코드 버그가 아니라 **여러 라운드에 걸쳐 조각
zip으로 전달한 파일들이 실제 작업 트리에 반영되는 순서가 어긋나서** 일부 파일이 예전
버전으로 남아있던 것으로 확인됨(제 샌드박스에서 동일 시나리오 실행 시 정상 작동 확인
후, 관련 파일 7개를 완전한 전체 파일로 재전달해서 해결됨 — 사장님이 실제로 적용 후 정상
동작 확인).
...
### 테스트 결과
전체 backend 737/737 통과(무변경). 프론트 빌드 성공(79 모듈).

### 남은 것
- `OPENAI_CASUAL_MODEL` 설정은 사장님이 Render에서 직접 진행 필요(코드 변경 아님).
- `autoBuyCode` 자동결제 경로에서 에러(Toss 키 미설정 등) 발생 시, 사용자가 별도
  "상품 목록 보기"로 돌아갈 방법이 아직 없음 — 다음에 개선 여지.

## [Unreleased] — 실제 발견한 심각한 버그: 모든 intent가 동일한 fallback으로 수렴

### [ROOT CAUSE]
`handleServiceIntentMessage`(Priority3에서 작성)의 `!userId` 분기가 `service_start`/
`product_question`/`payment_question` **세 가지 intent를 전혀 구분하지 않고 완전히
동일한 텍스트**를 반환하고 있었다 — 오직 `signup_question`만 다른 문구였다. 그래서
"사주 한 번 봐줘"와 "가격 얼마야?"가 사용자 눈에 똑같은 응답으로 보였다.

더 심각한 문제: 그 fallback 텍스트가 **"우선 네 생년월일이 있어야 제대로 들여다볼 수
있어"**로 시작하는데, 이 conversation은 애초에 `POST /api/charts`(생년월일 입력)가 먼저
끝나야 생성될 수 있는 구조라, **이 시점엔 이미 생년월일이 항상 존재한다.** 즉 실제로
막힌 이유(로그인 필요)와 완전히 다른 거짓 안내를 하고 있었다.

세 번째 문제: `product_question`/`payment_question`도 로그인 여부로 완전히 차단되고
있었는데, **가격/상품 정보는 원래 로그인 없이도 볼 수 있어야 하는 공개 정보**다(실제
구매 시점에만 로그인이 필요하고, 그 체크는 이미 `App.jsx`의 `onNeedPurchase`가 카드
버튼 클릭 시점에 정확히 하고 있었다). 그래서 가격을 물어봐도 실제 상품 카드 대신 매번
같은 회원가입 유도문만 나오고 있었다.

네 번째 문제(사장님이 직접 지적): 응답 문장을 `\n\n`(문단 하나)로만 묶어서 보내고 있어서,
프론트의 `splitIntoBubbles`(줄바꿈 기준 우선 분리)가 각 문단을 통째로 하나의 말풍선으로
만들었다 — 문단 안에 여러 문장이 뭉쳐서 "긴 말풍선"이 되던 원인.

### [FIX]
`handleServiceIntentMessage`를 intent별로 완전히 분리:
- **`product_question`/`payment_question`**: 로그인 여부와 무관하게 항상
  `listActiveProducts()`로 실제 가격을 조회해서 상품 카드로 응답(문구만
  "가격은 두 가지가 있어."/"여기서 바로 고르면 돼."로 구분). DB 조회 실패 시(이 환경처럼
  DB 없음) 명확한 에러로 안전하게 차단, 가짜 카드 생성 없음.
- **`service_start`(비로그인)**: "좋아. 제대로 한번 들여다보자. / 간단하게 가입만 하면
  바로 시작할 수 있어." — 생년월일 언급 완전히 제거, 로그인만 명확히 안내.
- **`signup_question`(비로그인)**: "응, 네 정보를 저장해두려면 간단한 가입이 필요해. /
  가입해두면 네가 본 사주를 나중에 다시 찾아볼 수 있어." — service_start와 명확히 다른
  문구.
- **로그인 상태에서 service_start/signup_question**: "이미 가입돼 있으니 바로 시작할 수
  있어."로 자연스럽게 상품 카드로 이어감(다시 가입을 요구하지 않음).
- 모든 응답 텍스트를 문장 하나당 한 줄(`\n`)로 작성 — 기존 `splitIntoBubbles` 로직은
  전혀 안 건드리고, 백엔드가 만드는 텍스트 형식만 고쳐서 짧은 말풍선 여러 개로 정확히
  분리되게 함.

### 실제 브라우저로 검증
- "사주 한 번 봐줘" → "좋아. 제대로 한번 들여다보자." / "간단하게 가입만 하면 바로 시작할
  수 있어." **두 개의 짧은 말풍선으로 정확히 분리**됨을 스크린샷으로 확인.
- "가입해야 돼?" → **A와 완전히 다른 문구**("응, 네 정보를 저장해두려면...") 확인.
- "가격 얼마야?" → 실제 DB 조회를 시도(이 환경엔 DB 없어 명확한 에러 배너로 안전하게
  차단, 서버 크래시 없음 — `/health` 정상 확인). 실제 배포 환경(DB 있음)에서는 이 지점에
  990/4900원 실제 카드가 뜰 것.

### 기존 계약 변경 반영
`tests/66-hybrid-intent-routing.test.mjs`의 테스트 4번 — "어디서 결제해?"가 이제
로그인 여부와 무관하게 실제 상품 카드를 시도하므로(이전엔 무조건 로그인 안내), 이
환경에선 DATABASE_URL 에러로 정상 차단되는 것을 검증하도록 업데이트(검증 약화 아님,
새로운 올바른 계약을 반영).

### 변경 파일
- `apps/api/src/services/conversation-service.mjs` — `handleServiceIntentMessage` 전면
  재작성.
- `tests/66-hybrid-intent-routing.test.mjs` — 테스트 4번 업데이트.

### 테스트 결과
전체 backend **737/737 통과**(1개 테스트 정직하게 계약 갱신, 재실행 확인 — 195번 flaky는
기존에 알려진 타이밍 의존 테스트). 프론트 빌드 성공(79 모듈, 이번 라운드는 백엔드 전용).

### Intent별 최종 동작표
| 사용자 발화 | intent | 비로그인 응답 | 로그인 응답 |
|---|---|---|---|
| "사주 한 번 봐줘" | service_start | 대구 말투 + signup_cta 카드 | 대구 말투 + 상품 카드 |
| "가격 얼마야?" | product_question | 실제 상품 카드(로그인 무관) | 동일 |
| "어디서 결제해?" | payment_question | 실제 상품 카드(로그인 무관) | 동일 |
| "가입해야 돼?" | signup_question | 대구 말투 + signup_cta 카드 | "이미 가입돼 있어" + 상품 카드 |
| "생년월일 입력하면 돼?" | (null, casual) | Casual API(생년월일 존재 컨텍스트 포함) | 동일 |

### 남은 문제
로그인 상태에서 실제 990/4900원 카드가 나오는지는 이 환경엔 DB가 없어 브라우저로 직접
확인 못 함 — 사장님 실제 배포 환경에서 재확인 필요.
